...
public class Holst {
    ...
    private final JButton but10 = new JButton("Массив фигур");
    ...
    private final JButton but18 = new JButton("Контейнер");
    private final JButton but19 = new JButton("На базе динамического массива");
    private final JButton but20 = new JButton("На базе линейного динамического списка");
    private final JButton but21 = new JButton("Добавить");
    ...
    private ContMassive contMassive = null;
    private ContList contList = null;
    ...
    public JPanel northPanel() {
        ...
        but5.addActionListener(e -> {
            ...
            if (numbut == 2) {
                ...
                if (contMassive == null) {
                    contMassive = new ContMassive();
                    contMassive.operation(1);
                    counterMassive = contMassive.getCount();
                    JOptionPane.showMessageDialog(h, "Создан Контейнер-Массив из " + counterMassive + " элементов");
                    cP.revalidate();
                    cP.repaint();
                }
                else {
                    JOptionPane.showMessageDialog(h, "Контейнер на базе массива уже создан");
                }
            }

            else if (numbut == 18) {
                if (contList == null) {
                    contList = new ContList(); // создание пустого контейнера
                    contList.operation(1);
                    counterList = contList.getCount();
                    JOptionPane.showMessageDialog(h, "Создан Контейнер-Список из " + counterList + " элементов");
                    cP.revalidate();
                    cP.repaint();
                }
                else {
                    JOptionPane.showMessageDialog(h, "Контейнер на базе списка уже создан");
                }
            }
        ...
        but6.addActionListener(e -> {
            ...
            if (numbut == 2) {
                ...
                if (contMassive != null) {
                    contMassive.operation(2);
                    int i = contMassive.getIMass();
                    for (int j = 0; j < i; j++) {
                        cP.add(contMassive.getFigureFromContMass(j), BorderLayout.CENTER);
                        cP.validate();
                    }
                    cP.revalidate();
                    cP.repaint();
                    ...
                    JOptionPane.showMessageDialog(h,"Фигуры не найдены");
                }
            }

            else if (numbut == 18) {
                if (contList != null) {
                    contList.operation(2);
                    int i = contList.getIList();
                    for (int j = 0; j < i; j++) {
                        cP.add(contList.getFigureFromContList(j), BorderLayout.CENTER);
                        cP.validate();
                    }
                    cP.revalidate();
                    cP.repaint();
        ...
        but14.addActionListener(e -> {
            if (numbut == 16) {
                ...
                if (contMassive != null) {
                    contMassive.operation(3);
                    counterMassive = contMassive.getCount();
                    for (int i = 0; i < counterMassive; i++) {
                        cP.add(contMassive.getFigureFromContMass(i), BorderLayout.CENTER);
                        cP.validate();
                        cP.revalidate();
                        cP.repaint();
                    }
                    contMassive.setIMass(counterMassive);
                } else {
                    JOptionPane.showMessageDialog(h, "Контейнер на базе массива не создан");
                }
            }
            else if (numbut == 18) {
                if (contList != null) {
                    contList.operation(3);
                    counterList = contList.getCount();
                    for (int i = 0; i < counterList; i++) {
                        cP.add(contList.getFigureFromContList(i), BorderLayout.CENTER);
                        cP.validate();
                        cP.revalidate();
                        cP.repaint();
                    }
                    contList.setIList(counterList);
                }  else {
                    JOptionPane.showMessageDialog(h, "Контейнер на базе списка не создан");
                }
            }
        ...
        but15.addActionListener(e -> {
            if (numbut == 16) {
                ...
                if (contMassive != null) {
                    contMassive.operation(4);
                    counterMassive = contMassive.getCount();
                    for (int i = 0; i < counterMassive; i++) {
                        cP.remove(contMassive.getFigureFromContMass(i));
                    }
                    cP.revalidate();
                    cP.repaint();
                    contMassive.setIMass(0);
                } else {
                    JOptionPane.showMessageDialog(h, "Контейнер на базе массива не создан");
                }
            }
                ...
                if (contList != null) {
                    contList.operation(4);
                    counterList = contList.getCount();
                    for (int i = 0; i < counterList; i++) {
                        cP.remove(contList.getFigureFromContList(i));
                    }
                    cP.revalidate();
                    cP.repaint();
                    contList.setIList(0);
        ...
        but16.addActionListener(e -> {
            if (numbut == 16) {
                ...
                if (contMassive != null) {
                    ...
                    contMassive.operation(5);
                    counterMassive = contMassive.getCount();
                    cP.revalidate();
                    cP.repaint();
                    contMassive = null;
                    JOptionPane.showMessageDialog(h, "Фигуры контейнера-массива стерты, а соответствующие объекты уничтожены");
                } else {
                    JOptionPane.showMessageDialog(h, "Контейнер на базе массива не создан");
                }
            }
                ...
                if (contList != null) {
                    ...
                    contList.operation(5);
                    counterList = contList.getCount();
                    cP.revalidate();
                    cP.repaint();
                    contList = null;
                    JOptionPane.showMessageDialog(h, "Фигуры контейнера-списка стерты, а соответствующие объекты уничтожены");
                } else {
                    JOptionPane.showMessageDialog(h, "Контейнер на базе списка не создан");
                }
            }
